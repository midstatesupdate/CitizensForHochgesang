...
- [`event.ts`](event.ts): Event documents with scheduling, location, description, slug, RSVP URL, event detail-page rich body/link fields, and per-event list card controls (layout variant, image orientation, image aspect ratio, animation preset, description preview character limit, and tags for related-event filtering). Includes `endDate` validation (`>= startDate`) so the website can automatically mark events as passed after the configured end timestamp.
