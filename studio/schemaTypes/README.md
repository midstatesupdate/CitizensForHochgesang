...
- [`post.ts`](post.ts): News/article documents with title, slug, excerpt, publish date, image, tags, plus per-article news list controls (card layout variant, image orientation, image aspect ratio, card animation preset, and body preview character limit) and scene-based body blocks (`Story Scene`, `Stat Callout`, inline images) for interactive reading experiences.
