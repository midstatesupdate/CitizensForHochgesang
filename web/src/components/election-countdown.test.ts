...
import {computeTimeLeft, padValue, resolveTimers, pickDefault} from './election-countdown'
...
describe('resolveTimers', () => {
  ...
  it('excludes timers whose expireDate is in the past', () => {
    const result = resolveTimers(
      [timer({expireDate: '2026-01-01T00:00:00Z'})],
      now,
    )
    expect(result).toHaveLength(0)
  })

  it('includes timers whose target is past but expireDate is future', () => {
    const result = resolveTimers(
      [timer({targetDate: '2026-05-01T00:00:00Z', expireDate: '2026-07-01T00:00:00Z'})],
      now,
    )
    expect(result).toHaveLength(1)
  })

  it('defaults expireDate to targetDate when absent', () => {
    const future = timer({targetDate: '2026-12-01T00:00:00Z', expireDate: null})
    const result = resolveTimers([future], now)
    expect(result).toHaveLength(1)
    expect(result[0]._expire.toISOString()).toBe(new Date('2026-12-01T00:00:00Z').toISOString())
  })
...
describe('pickDefault', () => {
  ...
  it('picks the timer with the nearest expire date', () => {
    const timers = [
      timer({heading: 'Far', targetDate: '2026-12-01T00:00:00Z'}),
      timer({heading: 'Near', targetDate: '2026-08-01T00:00:00Z', expireDate: '2026-08-02T00:00:00Z'}),
  ...
  it('breaks ties by returning the first in sorted order', () => {
    ...
    const resolved = resolveTimers(timers, now)
    const idx = pickDefault(resolved)
    // Both have same expire, first in array (index 0 after sorting) wins
    expect(idx).toBe(0)
  })
})
